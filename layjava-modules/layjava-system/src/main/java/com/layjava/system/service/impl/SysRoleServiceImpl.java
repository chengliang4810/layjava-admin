...
public class SysRoleServiceImpl implements SysRoleService {
    ...
    public void checkRoleAllowed(SysRoleBo role) {
        ...
        if (ObjectUtil.isNull(role.getRoleId())
            && StrUtil.equalsAny(role.getRoleKey(), keys)) {
        ...
        if (ObjectUtil.isNotNull(role.getRoleId())) {
            ...
            if (!StrUtil.equals(sysRole.getRoleKey(), role.getRoleKey())) {
                ...
                } else if (StrUtil.equalsAny(role.getRoleKey(), keys)) {
                    throw new ServiceException("不允许使用系统内置管理员角色标识符!");
                }
            }
        }
    ...
    public void cleanOnlineUserByRole(Long roleId) {
        ...
        keys.parallelStream().forEach(key -> {
            String token = null; // StringUtils.substringAfterLast(key, ":");
            // 如果已经过期则跳过
    ...
    // @Override
    public List<Long> selectUserIdsByRoleId(List<Long> roleIds) {
        ...
        return SimpleQuery.list(lqw, SysUserRole::getUserId);
    }
